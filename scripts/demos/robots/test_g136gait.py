...
# Pre-defined configs
##
from isaaclabex.assets.robots.unitree import G1_CFG
...
def update_robots(robot: Articulation, _phases: torch.Tensor, sim_dt: float):
    _scales = torch.tensor([
        0.25, 0.25,  # 0 ## hip_pitch_joint
        0, 0,  # 2
        0, 0,  # 4
        0, 0,  # 6
        0, 0,  # 8
        0,     # 10
        0.5, 0.5,  # 11 ## knee_joint
        0, 0,  # 13
        0, 0,  # 15
        0, 0,  # 17
        0, 0,  # 19
...
def design_scene(sim: sim_utils.SimulationContext) -> tuple[list, torch.Tensor]:
    ...
    # Robots
    G1_CFG.init_state.pos = (0.0, 0.0, 1)
    G1_CFG.spawn.articulation_props.fix_root_link = True
